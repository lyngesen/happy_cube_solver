# credit: https://github.com/mikepound/cubes
# script for generating a set of polycubes using n cubes
# see https://www.youtube.com/watch?v=g9n0a0644B4
...
def generate_polycubes(n, use_cache=True):
    """
    ...
    # Check cache
    cache_path = f"./polycube_cache/polycubes_{n}.npy"
...
def get_coords_cubegrid(polycube):
    ...
    for x in range(D[0]):
        for y in range(D[1]):
            if polycube[x,y] == 1:
                cubes_coords.append((x,y))

    return cubes_coords



def generate_cube_coords(n):

    all_cubes = list(generate_polycubes(n))
    coord_list = [get_coords(cube) for cube in all_cubes]
    return coord_list 
